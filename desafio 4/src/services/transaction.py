...
class TransactionService:
    async def read_all(
        ...
        query = transactions.select().where(transactions.c.account_id ==
                                            account_id
    ...
    async def create(self, transaction: TransactionIn) -> Record:
        query = accounts.select().where(
            accounts.c.id == transaction.account_id
        )
        ...
        if transaction.type == TransactionType.WITHDRAWAL:
            ...
            if balance < 0:
                raise BusinessError(
                    "Operation not carried out due to lack of balance")
        else:
            balance = float(account.balance) + transaction.amount
    ...
    async def __update_account_balance(
        ...
        command = accounts.update().where(
            accounts.c.id == account_id).values(balance=balance)
